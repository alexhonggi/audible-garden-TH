...
from utils.audio_utils_simple import generate_midi_from_roi, send_midi
...
class TurntableGUI(QMainWindow):
    ...
    def process_logic(self):
        ...
        if self.osc_client and len(midi_notes) > 0:
            send_midi(self.osc_client, len(midi_notes), midi_notes, velocities, durationss)
    ...
    while True:
        ...
        if roi_gray.size > 0:
            ...
            if osc_client and len(midi_notes) > 0:
                send_midi(osc_client, midi_notes, velocities, durations)
